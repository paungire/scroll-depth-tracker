...
Нативный JS-скрипт для отслеживания глубины просмотра страницы. Разбивает страницу на равные части и срабатывает при достижении каждой границы — без зависимостей, без jQuery.
...
По умолчанию трекер работает только на `/`. На Тильде это защищает от срабатывания на странице «Спасибо за заявку»:

```js
// Только главная
new ScrollDepthTracker({ pages: ['/'] });

// Главная и страница с формой
new ScrollDepthTracker({ pages: ['/', '/order'] });

// Все страницы (не рекомендуется)
new ScrollDepthTracker({ pages: [] });
```
...
// В течение 1 часа
new ScrollDepthTracker({ ttl: 3600 });

// В течение 1 дня
new ScrollDepthTracker({ ttl: 86400 });

// В течение 1 недели
new ScrollDepthTracker({ ttl: 604800 });
```
